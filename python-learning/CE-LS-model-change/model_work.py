...
def train_layer(layer,pi):
    ...
    x_ = pd.read_csv('x_'+pi+'_val.csv')
    H_ = pd.read_csv('y_'+pi+'_CSI_val.csv')
    D_ = pd.read_csv('y_'+pi+'_data_val.csv')
...
#******************************************************再训练***************************************************************
def train_layer_again(layer,pi):
    [out, w_1, b_1, w_2, b_2] = choose_layer(layer, xs)
    regularizer = tf.contrib.layers.l2_regularizer(theta)  # 正则化参数
    reg_term = tf.contrib.layers.apply_regularization(regularizer)
...
def save_layer(layer,pi):
    ...
    df_w1.to_csv('wh_'+pi+'_'+L_str+'_1.csv')
    df_b1.to_csv('bh_'+pi+'_'+L_str+'_1.csv')
    df_w2.to_csv('wh_'+pi+'_'+L_str+'_2.csv')
    df_b2.to_csv('bh_'+pi+'_'+L_str+'_2.csv')
    print("save  "+L_str +"-"+ pi +"  ,OK")
