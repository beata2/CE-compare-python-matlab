...
walsh = pd.read_csv('Walsh_matrix512x64.csv')  # 1024x32
...
H_Net_1 = np.array([2*CSI_len, 8*CSI_len, 2*CSI_len])
D_Net_1 = np.array([2*L_len,8*L_len,2*L_len])
...
def choose_layer(layer,xs):
    if layer == 1:
        h_1 = despreading(xs)
        [out, w_1, b_1, w_2, b_2] = (H_DNN(h_1))
    elif layer == 2:
        h_1 = despreading(xs)
        [H_1_1, w_1, b_1, w_2, b_2] = (H_DNN_ok(h_1,layer-1,"1"))
        [H_2_1, w_1, b_1, w_2, b_2] = (H_DNN_ok(h_1, layer - 1, "2"))
        H_1 = 0.5*(H_1_1+H_2_1)
        d_1 = (tf.subtract(xs, tf.multiply(Const_h, spreading(H_1))))
        [out, w_1, b_1, w_2, b_2] = (D_DNN(d_1))
    elif layer == 3:
        h_1 = despreading(xs)
        [H_1_1, w_1, b_1, w_2, b_2] = (H_DNN_ok(h_1,layer-2,"1"))
        [H_2_1, w_1, b_1, w_2, b_2] = (H_DNN_ok(h_1, layer - 2, "2"))
        H_1 = 0.5*(H_1_1+H_2_1)
        d_1 = (tf.subtract(xs, tf.multiply(Const_h, spreading(H_1))))
        [D_1_1, w_1, b_1, w_2, b_2] = (D_DNN_ok(d_1,layer-1,"1"))
        [D_2_1, w_1, b_1, w_2, b_2] = (D_DNN_ok(d_1,layer-1,"2"))
        D_1 = 0.5 * (D_1_1 + D_2_1)
        h_2 = despreading(tf.subtract(xs, tf.multiply(Const_d, D_1)))
        [out, w_1, b_1, w_2, b_2] = (H_DNN(h_2))
    elif layer == 4:
        h_1 = despreading(xs)
        [H_1_1, w_1, b_1, w_2, b_2] = (H_DNN_ok(h_1,layer-3,"1"))
        [H_2_1, w_1, b_1, w_2, b_2] = (H_DNN_ok(h_1, layer-3, "2"))
        H_1 = 0.5*(H_1_1+H_2_1)

        d_1 = (tf.subtract(xs, tf.multiply(Const_h, spreading(H_1))))
        [D_1_1, w_1, b_1, w_2, b_2] = (D_DNN_ok(d_1,layer-2,"1"))
        [D_2_1, w_1, b_1, w_2, b_2] = (D_DNN_ok(d_1,layer-2,"2"))
        D_1 = 0.5 * (D_1_1 + D_2_1)

        h_2 = despreading(tf.subtract(xs, tf.multiply(Const_d, D_1)))
        [H_1_2, w_1, b_1, w_2, b_2] = (H_DNN_ok(h_2,layer-1,"1"))
        [H_2_2, w_1, b_1, w_2, b_2] = (H_DNN_ok(h_2,layer-1,"2"))
        H_2 = 0.5*(H_1_2+H_2_2)

        d_2 = (tf.subtract(xs, tf.multiply(Const_h, spreading(H_2))))
        [out, w_1, b_1, w_2, b_2] = (D_DNN(d_2))
    elif layer ==5:
        # 做为测试模型使用
        h_1 = despreading(xs)
        [H_1_1, a, b, c, d] = (H_DNN_ok(h_1,layer-4,"1"))
        [H_2_1, a, b, c, d] = (H_DNN_ok(h_1,layer-4,"2"))
        H_1 = 0.5*(H_1_1+H_2_1)

        d_1 = (tf.subtract(xs, tf.multiply(Const_h, spreading(H_1))))
        [D_1_1, a, b, c, d] = (D_DNN_ok(d_1,layer-3,"1"))
        [D_2_1, a, b, c, d] = (D_DNN_ok(d_1,layer-3,"2"))
        D_1 = 0.5 * (D_1_1 + D_2_1)

        h_2 = despreading(tf.subtract(xs, tf.multiply(Const_d, D_1)))
        [H_1_2, a, b, c, d] = (H_DNN_ok(h_2,layer-2,"1"))        # **********为方便起见，这里的H_2用w_1代替*********
        [H_2_2, a, b, c, d] = (H_DNN_ok(h_2,layer-2,"2"))        # **********为方便起见，这里的H_2用w_1代替*********
        w_1 = 0.5*(H_1_2+H_2_2)

        d_2 = (tf.subtract(xs, tf.multiply(Const_h, spreading(w_1))))
        [D_1_2, a, b_1, w_2, b_2] = (D_DNN_ok(d_2,layer-1,"1"))
        [D_2_2, a, b_1, w_2, b_2] = (D_DNN_ok(d_2,layer-1,"2"))
        out =0.5*(D_1_2+D_2_2)
...
def H_DNN_ok(input,layer,pi):
    L_str = str(layer)
    w_1 = np.float32(pd.read_csv('wh_'+pi+'_'+L_str+'_1.csv').ix[:,1:])
    b_1 = np.float32(pd.read_csv('bh_'+pi+'_'+L_str+'_1.csv').ix[:,1:])

    layer_1 = tf.nn.tanh((tf.add(tf.matmul(batch_norm(input), w_1), b_1)))
    w_2 = np.float32(pd.read_csv('wh_'+pi+'_'+L_str+'_2.csv').ix[:,1:])
    b_2 = np.float32(pd.read_csv('bh_'+pi+'_'+L_str+'_2.csv').ix[:,1:])
    layer_2 = (tf.add(tf.matmul((layer_1),w_2),b_2))
    return [layer_2,w_1,b_1,w_2,b_2]

def D_DNN_ok(input,layer,pi):
    L_str= str(layer)
    w_1 = np.float32(pd.read_csv('wh_'+pi+'_'+L_str+'_1.csv').ix[:,1:])
    b_1 = np.float32(pd.read_csv('bh_'+pi+'_'+L_str+'_1.csv').ix[:,1:])
...
def gen_data(fun,pi):
    ...
    print(np.shape(df2))
    print(np.shape(df3))
    df1.to_csv('x_'+pi+'_val.csv')
    print("导出x_"+pi+"_val")
    df2.to_csv('y_'+pi+'_CSI_val.csv')
    print("导出y_"+pi+"_CSI_val")
    df3.to_csv('y_'+pi+'_data_val.csv')
    print("导出y_"+pi+"_data_val")
